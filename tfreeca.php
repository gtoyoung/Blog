...
function download_impl() {
	...
	preg_match("/.click.*'(?<page>.*\.php)'/s", $data, $match);
	$downloadUrl = $file_domain.$match[page];
	
	//preg_match("/\/(?<link>link.php\?[^\"]*)/", $data, $match);
	$count = preg_match_all("/<input.*name=\"(?<name>[^\"]*)\".*value=\"(?<value>[^\"]*)\"[^>]*/", $data, $match);
	...
	for($i = 0; $i < $count; $i++){
		...
		$param = $param.$match[name][$i].'='.$match[value][$i];
	}
	
	if( preg_match("/<form.*method=\"post\"/i", $data, $match) > 0 ){
		$data = get_html($downloadUrl,  $headers, true, $param);
	} else {
...
function get_html($url, $headers, $isPost=false, $postField=null) {
	...
	curl_setopt($ch, CURLOPT_COOKIEJAR, '/tmp/filetendercookie.txt');
	curl_setopt($ch, CURLOPT_COOKIEFILE, '/tmp/filetendercookie.txt');
